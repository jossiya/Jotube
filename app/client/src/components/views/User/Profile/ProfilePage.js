...
import NickName from './sections/NickName';
 ...
 function ProfilePage(props) {
  ...
  const user= useSelector(state=> state.user)
    ...
    console.log('기본 닉네임',NickNameChange,user.userData?.nickname)
      //처음 랜더링 유저 사진 띄운 것d
      useEffect(() => {
        if(user){
          if(user&&user.userData&&user.userData.email){
            setFilePath(user.userData.image)
            setNickNameChange(user.userData.nickname)
      ...
      console.log('변경할 닉네임', NickNameChange)
    const onSubmit =(e)=>{
      e.preventDefault();
        const variable={
          FilePath:FilePath,
      ...
      return (
        <div >
            ...
            <Dropzone
              ...
              <br/>
              {/* 닉네임 변경 */}
              <NickName NickNameChange={setNickNameChange}/>
                  <br/>
                  <hr/>
                {/* 정보전송 */}
