...
>>🔥구독 : 구독 버튼(자신동영상은 보이지 않게함)을 클릭하면 유저의 id와 writer의 id가 DB로 저장이 되고 페이지 랜더 시 writer의 id로 구독자 테이블을 검색하여 배열의 갯수가 구독자 수로 나와 표시됩니다.<br/>
>>댓글 : 댓글 과 답글로 나누어 컨포넌트를 만들 었고, 댓글 작성자와 답글 작성자 id와 댓들 자체 개별 id를 만들고 videoid에 저장(DB에)하여 댓글들이 보이게 하였고 댓글에 단 답글은 코댓글 id를 이용하여 출력되게 했습니다. <b>videoid>commentid>responseTo</b><br/> 
>>좋아요:<br/> 
...
#### :dizzy:프로필 변경 페이지
:fire:Dropzone을 이용하여 프로필 업로드 및 변경,선택 취소 가 가능하도록 기능을 구현 하였으며, 별명을 수정 할 수 있습니다.
